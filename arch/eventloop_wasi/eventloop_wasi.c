...
#include "eventloop_wasi.h"
#include "open62541/plugin/eventloop.h"

#if defined(UA_ARCHITECTURE_WASI) 
#include <time.h>
#endif
...
UA_EventLoopPOSIX_start(UA_EventLoopPOSIX *el) {
    ...
    const UA_Int32 *csm = (const UA_Int32*)
        UA_KeyValueMap_getScalar(&el->eventLoop.params,
                                 ...
                                 &UA_TYPES[UA_TYPES_INT32]);
#if defined(UA_ARCHITECTURE_WASI) 
    el->clockSource = (UA_Int32)CLOCK_REALTIME;
    if(cs)
        el->clockSource = *cs;

# ifdef CLOCK_MONOTONIC_RAW
    el->clockSourceMonotonic = (UA_Int32)CLOCK_MONOTONIC_RAW;
# else
    el->clockSourceMonotonic = (UA_Int32)CLOCK_MONOTONIC;
# endif
    if(csm)
